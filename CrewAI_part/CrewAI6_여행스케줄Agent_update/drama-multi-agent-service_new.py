...
content_reporter = Agent(
    role='컨텐츠 기자',
    goal='드라마의 각 에피소드 내용을 하이라이트 위주로 요약 정리, 주요한 포인트와 드라마의 분위기를 잘 드러냅니다',
    backstory='10년 경력의 드라마 전문 기자로, 팩트 위주로, 정확한 에피소드 요약을 제공합니다.',
    verbose=True,
...
youtube_creator = Agent(
    ...
    backstory='5년 경력의 유튜브 크리에이터로, 드라마 팬들의 관심을 끄는 영상 컨텐츠를 기획합니다.',
    verbose=True,
    allow_delegation=False,
    llm=gpt
)
...
if 'drama_title' not in st.session_state:
    st.session_state.drama_title = "엄마친구아들"
...
if st.sidebar.button("드라마 분석 시작"):
    # 태스크 1: 드라마 내용 요약
    task1 = Task(
        description=f"{st.session_state.drama_title}의 1화부터 {st.session_state.current_episode}화까지의 내용을 YouTube에서 검색하고 자막을 가져와 각 에피소드별로 간단히 요약해주세요. 전체 자막을 분석하고 중요한 내용만 추려서 요약해주세요.",
    ...
    # 태스크 3: 유튜브 컨텐츠 기획
    task3 = Task(
        description=f"{st.session_state.drama_title}의 {next_episode}화 예상 내용을 바탕으로 흥미로운 10분 내외의 유튜브 영상 스크립트를 작성해주세요. 드라마 팬들의 관심을 끌 수 있는 내용으로 구성해주세요.",
        agent=youtube_creator,
        expected_output="지난 화의 내용을 짧게 요약하고, 다음화의 예상을 정리한 스크립트, 주요 예측 포인트, 팬들의 관심을 끌 수 있는 요소"
    )
